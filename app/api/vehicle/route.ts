...
function clean(val: unknown) {
  ...
  if (!s || s === "0" || s === "Not Applicable") return null;
  return s;
}
...
export async function GET(req: Request) {
  ...
  const query = new URLSearchParams({ format: "json" });
  if (modelYear) query.set("modelyear", modelYear);

  // must be put in env variable later
  const endpoint = `https://vpic.nhtsa.dot.gov/api/vehicles/DecodeVinValuesExtended/${encodeURIComponent(
    vin
  )}?${query.toString()}`;
  ...
  try {
    ...
    const engine = clean(row.EngineModel) ?? clean(row.EngineConfiguration);
